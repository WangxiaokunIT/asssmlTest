...
> * spring boot升级到最新的2.1.2版本！
> * quertz升级到最新的2.3.0版,优雅的实现与spring boot继承。
> * 新增DictField属性注解,用于和字典的关联查询。
> * 新增JoinField属性注解,用于关联查询。
> * 新增dictSelector标签,用于字典项的渲染。
> * 新增dictSelectorCon标签,用于字典项的查询条件渲染。
> * 新增树状结构表level,seq等公共字段的维护方法。
> * 新增model中公共属性如创建人创建时间等属性的自动插入值与查询值功能。
> * 新增lombok插件,减少get、set等方法的代码量。
> * 新增职位表用于更方便的管理数据权限。
> * 新增文件管理功能用于更方便的管理上传文件。
> * 修改系统中用户角色，建立用户角色中间表来保存用户角色。
> * 修改系统管理表字段使之更规范化。
> * 修改系统中的html标签，使之更简单更规范。
> * 修改数据列表显示由单行选择改为多行选择。
> * 修改数据列表分页方式由前台分页改为后台分页。
> * 去掉部分包装类，改为属性注解方式级联查询。
> * 修复新增、修改页日期字段为空时系统报错的bug。
...
6. controller层采用map + warpper方式的返回结果，返回给前端更为灵活的数据，具体参见com.zhangjiajia.asmall.modular.system.warpper包中具体类。
...
9. 控制器层统一的异常拦截机制,利用@ControllerAdvice统一对异常拦截,具体见com.zhangjiajia.asmall.core.aop.GlobalExceptionHandler类。
10. 页面统一的js key-value单例模式写法，每个页面生成一个唯一的全局变量，提高js的利用效率，并且有效防止多个人员开发引起的函数名/类名冲突，并且可以更好地去维护代码。

## 基于javabean方式的spring配置
asmall以简洁为核心,抛弃了传统的易错,臃肿xml配置,采用javabean的方式配置spring,简化了项目的配置,如下示例为配置mybatis-plus和数据源:
```
@Configuration
@MapperScan(basePackages = {"com.zhangjiajia.asmall.modular.*.dao", "com.zhangjiajia.asmall.common.persistence.dao"})
...
## 极简的图片上传方法
asmall对web-upload进行二次封装,让图片的上传功能呢只用2行代码即可实现,如下
```
...
asmall的数据范围控制是指,对拥有相同角色的用户,根据部门的不同进行相应的数据筛选,如果部门不相同,那么有可能展示出的具体数据是不一致的.所以说asmall对数据范围控制是以部门id为单位来标识的,如何增加数据范围拦截呢?只需在相关的mapper接口的参数中增加一个DataScope对象即可,DataScope中有两个字段,scopeName用来标识sql语句中部门id的字段名称,例如deptiid或者id,另一个字段deptIds就是具体需要过滤的部门id的集合.拦截器原理如下:拦截mapper中包含DataScope对象的方法,获取其原始sql,并做一个包装限制部门id在deptIds范围内的数据进行展示.
