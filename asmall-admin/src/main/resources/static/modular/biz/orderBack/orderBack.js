...
OrderBack.initColumn = function () {
    return [
        ...
        {title: '退货物流公司', field: 'backCompanyName',width: '150px',  visible: true, align: 'center', valign: 'middle',formatter:OrderBack.paramsMatter},
...
OrderBack.searchTool = function(value, row) {
    if(row.state == 1&&row.backRemark=="未发货取消购买") {
        return '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\' , 3,\''+row.backRemark+ '\')">退款</a>'+' | ' + '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\' , 6)">拒绝</a>';
    }else if(row.state == 1) {
        return '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\' , 3)">审核</a>'+' | ' + '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\' , 6)">拒绝</a>';
    } else if (row.state == 6) {
        return '<a href="javascript:;" onclick="window.order_back_vue.showContent(\''+row.refuseRemark+'\')">【 查看原因 】</a>'
    }
}
...
OrderBack.openAddOrderBack = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/orderBack/orderBack_add'
    });
    this.layerIndex = index;
};
...
OrderBack.openOrderBackDetail = function () {
    if (this.check(false)) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/orderBack/orderBack_update/' + OrderBack.seItems[0].id
        });
        this.layerIndex = index;
    }
};
...
OrderBack.delete = function () {
    if (this.check(true)) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/orderBack/delete", function (data) {
                Feng.success("删除成功!");
                OrderBack.table.refresh();
            }, function (data) {
...
$(function () {
    ...
    var table = new BSTable(OrderBack.id, "/orderBack/pageList", defaultColunms);
    ...
    window.order_back_vue = new Vue({
        ...
        methods: {
             ...
             * @param s
             */
            btnClick: function(value, s,remark) {
                let this_ = this;
                if(s == 3&&remark=="未发货取消购买") {
                    Feng.confirm("是否确认退款？ ", function () {
                        this_.updateState({
                            id: value,
                            state: s
                        });
                    ...
                    Feng.confirm("是否确认审核？ ", function () {
                        window.modal_backaddress_vue.id = value;
                        window.modal_backaddress_vue.state = s;
                        window.modal_backaddress_vue.readonly = false;
                        window.modal_backaddress_vue.backSeller = '';
                        window.modal_backaddress_vue.backSellerPhone = '';
                        window.modal_backaddress_vue.backAddress = '';
                        $('#backaddressModal').modal('show');
                    });
                }else if(s == 3) {
                    Feng.confirm("是否确认审核？ ", function () {
                        window.modal_backaddress_vue.id = value;
            ...
            updateState: function(obj, fun) {
                window.vueUtils.ajax( obj, "/orderBack/update", function (data) {
                    let index = layer.alert('退货状态变更成功', {
                        skin: 'layui-layer-molv' //样式类名
                    }, function(){
                        layer.close(index);
                        $('#backaddressModal').modal('hide');
                        OrderBack.search();
