...
OrderBack.searchTool = function(value, row) {
    if(row.state == 12) {
        return '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\', 18)">退款</a>';
    }
    else if (row.state == 15) {
        return  '<a href="javascript:;" onclick="window.order_back_vue.btnClick(\''+row.id+'\' , 18)">确认退款</a>';
    }
    else {
        return '-';
    }
...
OrderBack.openAddOrderBack = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/orderBackRecive/orderBack_add'
    });
    this.layerIndex = index;
};
...
OrderBack.openOrderBackDetail = function () {
    if (this.check(false)) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/orderBackRecive/orderBack_update/' + OrderBack.seItems[0].id
        });
        this.layerIndex = index;
    }
};
...
OrderBack.delete = function () {
    if (this.check(true)) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/orderBackRecive/delete", function (data) {
                Feng.success("删除成功!");
                OrderBack.table.refresh();
            }, function (data) {
...
$(function () {
    ...
    var table = new BSTable(OrderBack.id, "/orderBackRecive/pageList", defaultColunms);
    ...
    window.order_back_vue = new Vue({
        ...
        methods: {
            ...
            updateState: function(obj, fun) {
                window.vueUtils.ajax( obj, "/orderBackRecive/update", function (data) {
