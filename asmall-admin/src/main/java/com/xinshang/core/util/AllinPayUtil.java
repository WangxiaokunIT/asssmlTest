...
public class AllinPayUtil {
     ...
     * @return
     */
    public <T> Optional<AllinPayResponseDTO<T>> request (YunRequest yunRequest, Class<T> clazz){
        log.info("开始请求通联接口:{}",yunRequest);
        AllinPayResponseDTO<T> apr = new AllinPayResponseDTO<>();
        try {
            ...
            if(Constants.SUCCESS_CODE.equals(apr.getStatus())){
    ...
    public <T> Optional<T> response (AllinPayAsynResponseDTO allinPayAsynResponseDTO, Class<T> clazz){
        ...
        try{
          ...
          /*  Map<String,String> map = JSON.parseObject(rps,Map.class);
            if (!Constants.SUCCESS_CODE.equals(map.get("status"))) {
