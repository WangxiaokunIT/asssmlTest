...
window.project_info_vue = new Vue({
    ...
    methods: {
        ...
        handleSearch:function(value){
            ...
            this.dataSource = [];
            let this_ = this;
            window.vueUtils.ajax( null, "/supplier/searchSupplier?search=" + value, function (data) {
                let arr = data.value;
                for (let k in arr) {
                    this_.dataSource.push(arr[k].searchContent)
                    this_.supplierDetail[arr[k].id] = arr[k];
                }
            });
        },
         ...
         */
        onSelect(value) {
            this.supplier = this.supplierDetail[parseInt(value.split('|')[0])];
            this.quota = this.supplier.loanLimit - (this.supplier.usedLoanAmount || 0);
         ...
         * 测试验证
         */
        validateTest: function(fun) {

            let c = CKEDITOR.instances.description.getData();
            console.log(c)
            console.log('--------------------------')
            console.log(c.replace(/style\s*?=\s*?([‘"])[\s\S]*?\1/ig, 'style="width:100%;height:auto;"'));
            ...
            if( parseFloat(this.obj.minMoney) < 0.01 || parseFloat(this.obj.minMoney) > 0.1 ) {
                parent.layer.msg('测试只能填写0.01~0.1');
                return;
            }

            if(this.obj.maxMoney != 0.1) {
                parent.layer.msg('测试环境最高只能0.1');
                return;
            }
            ...
            this.obj.details = c.replace(/style\s*?=\s*?([‘"])[\s\S]*?\1/ig, 'style="width:100%;height:auto;"');
        ...
        updateState: function (s, c) {
            ...
            window.vueUtils.ajax(
                ...
                , function (data) {
                    ...
                    parent.Project.search();
                    layer.close(this_.loading);
                    // Feng.error("操作失败!" + error.responseJSON.message + "!");
                    parent.layer.alert(error.responseJSON.message)
                })
        },
        ...
        btnClick: function (s) {
            ...
            // else if(s == 7) {
            //     let withdrawalWindow = layer.open({
            //         type: 2,
            //         title: '提现申请',
            //         area: ['800px', '600px'], //宽高
            //         fix: false, //不固定
            //         maxmin: true,
            //         content: "/supplier/openCashWithdrawalProject/" + this.supplier.id + "/" + this.obj.id
            //     });
        ...
        btnClickInit: function(s) {
            let this_ = this;
            Feng.confirm("<div style='color: red'> 只有  <b style='border-bottom: 1px solid red;padding-bottom: 1px;'>提现成功</b> ，但是  <b style='border-bottom: 1px solid red;padding-bottom: 1px;'>未生成还款计划</b>  时才能操作！是否确认变更状态和生成还款计划？</div> ", function () {
                this_.updateState(s);
            });
        },
