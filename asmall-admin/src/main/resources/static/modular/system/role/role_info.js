...
RolInfoDlg.onClickPName = function (e, treeId, treeNode) {
    // $("#parentNameContent").show()
    $("#parentName").attr("value", RolInfoDlg.pNameZtree.getSelectedVal());
...
RolInfoDlg.showPNameSelectTree = function () {
    Feng.showInputTree("parentName", "parentNameContent", "parentCategorySelect",15, 34);
};
...
RolInfoDlg.collectData = function () {
    this.set('id').set('name').set('parentId').set('remark').set('sortNum');
};
...
$(function () {
    ...
    var pNameTree = new $ZTree("parentIdTree", "/role/roleTreeList");
